...
function setup_hypc
{
    ...
    HYPCVERSION_SHORT=$(echo "$HYPCVERSION" | sed -En 's/^([0-9.]+).*\+commit\.[0-9a-f]+.*$/\1/p')
...
function replace_version_pragmas
{
    ...
    find . test -name '*.sol' -type f -print0 | xargs -0 sed -i -E -e 's/pragma hyperion [^;]+;/pragma hyperion >=0.0;/'
}
