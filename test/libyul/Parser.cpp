...
BOOST_AUTO_TEST_CASE(default_types_set)
{
	...
	shared_ptr<Block> result = parse(
		...
		"}",
		EVMDialectTyped::instance(ZVMVersion{}),
		reporter
	);
	...
	// should be omitted.
	BOOST_CHECK_EQUAL(AsmPrinter{EVMDialectTyped::instance(ZVMVersion{})}(*result),
		"{\n"
		"    let x:bool := true\n"
		"    let z:bool := true\n"
		"    let y := add(1, 2)\n"
...
BOOST_AUTO_TEST_CASE(customSourceLocations_empty_block)
{
	...
	auto const sourceText =
		"/// @src 0:234:543\n"
		"{}\n";
	EVMDialectTyped const& dialect = EVMDialectTyped::instance(ZVMVersion{});
