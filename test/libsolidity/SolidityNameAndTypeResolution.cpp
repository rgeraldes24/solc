	...
	char const* sourceCode = R"(
		contract C {
			function f() public view {
				address(Z4242).staticcall.value;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(address_call_full_return_type)
{
	char const* sourceCode = R"(
		contract C {
			function f() public {
				(bool success, bytes memory m) = address(Z4242).call("");
				success; m;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(address_delegatecall_full_return_type)
{
	char const* sourceCode = R"(
		contract C {
			function f() public {
				(bool success, bytes memory m) = address(Z4242).delegatecall("");
				success; m;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(address_staticcall_full_return_type)
{
	char const* sourceCode = R"(
		contract C {
			function f() public view {
				(bool success, bytes memory m) = address(Z4242).staticcall("");
				success; m;
			}
		}
	)";
