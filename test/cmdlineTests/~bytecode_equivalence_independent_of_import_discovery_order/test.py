...
def test_bytecode_equivalence():
    ...
    save_bytecode(Path('A.bin'), hypc_bin_report(hypc_binary, [Path('A.sol')], via_ir=True))
    save_bytecode(Path('AB.bin'), hypc_bin_report(hypc_binary, [Path('A.sol'), Path('B.sol')], via_ir=True))
