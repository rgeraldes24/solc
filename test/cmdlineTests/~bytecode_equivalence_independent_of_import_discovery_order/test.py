...
def test_bytecode_equivalence():
    ...
    save_bytecode(Path('A.bin'), solc_bin_report(solc_binary, [Path('A.hyp')], via_ir=True))
    save_bytecode(Path('AB.bin'), solc_bin_report(solc_binary, [Path('A.hyp'), Path('B.hyp')], via_ir=True))
