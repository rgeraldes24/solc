...
i.e., the output of ``hypc --bin contract.sol``, is generally difficult to read. It is recommended
...
The following would be the output of ``hypc --asm contract.sol``
...
contract.sol``. Often times, it is interesting to see if two different sources in Hyperion result in
